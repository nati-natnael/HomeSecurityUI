...
import Janus, { bootbox } from "janus-gateway-js";

var server = "http://192.168.0.8:8088/janus";

var janus = null;
var streaming = null;
var opaqueId = "streamingtest-test";
...
const pluginAttachSuccess = (handle) => {
  streaming = handle;
  Janus.log(
...
const pluginAttachError = (error) => {
  Janus.error("  -- Error attaching plugin... ", error);
};

const pluginIceStateChange = (state) => {
  Janus.log(`ICE state changed to ${state}`);
};

const pluginWebRTCStateChange = (state) => {
  Janus.log(
    `Janus says our WebRTC PeerConnection is ${state ? "up" : "down"} now`
  );
};

const pluginOnMessage = (msg, jsep) => {
...
const pluginOnRemoteStream = (stream) => {
...
const pluginOnData = (data) => {
  Janus.debug("We got data from the DataChannel!", data);
};

const pluginOnCleanUp = () => {
...
const createSessionSuccess = () => {
  ...
  janus.attach({
    ...
    opaqueId: opaqueId,
    success: pluginAttachSuccess,
    error: pluginAttachError,
    iceState: pluginIceStateChange,
    webrtcState: pluginWebRTCStateChange,
    onmessage: pluginOnMessage,
    onremotestream: pluginOnRemoteStream,
    ondataopen: pluginOnDataOpen,
    ondata: pluginOnData,
    oncleanup: pluginOnCleanUp,
  });
};
...
const createSessionDestroy = () => {
  window.location.reload();
};

const janusInitCallback = () => {
  // Use a button to start the demo
  document.getElementById("start").one("click", function () {
    ...
    janus = new Janus({
      ...
      destroyed: createSessionDestroy,
    });
  });
};
